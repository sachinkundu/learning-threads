  ...
  function validate(state) {
    ...
    for (const node of state.conversations) {
      ...
      if (node.sourceAnchor) {
        const a=node.sourceAnchor;
        check(validId(a.id) && a.book===BOOK && a.page===node.page && typeof a.quote==='string' && Number.isInteger(a.start) && a.start>=0 && a.end===a.start+a.quote.length && a.quote.length>0, 'Saved source highlight is invalid.');
      }
    ...
    return {
      load() {
        ...
        try {
          ...
          check(envelope.version===1 && envelope.book===BOOK && Number.isSafeInteger(envelope.revision) && envelope.revision>0, 'Saved study format is not supported.');
      ...
      save(state) {
        ...
        const next=JSON.stringify({version:1,book:BOOK,revision:revision+1,updatedAt:new Date().toISOString(),state});
      ...
      backup(state) {
        ...
        return {version:1,book:BOOK,exportedAt:new Date().toISOString(),state,stored,storageError:error};
      }
    };
  }
